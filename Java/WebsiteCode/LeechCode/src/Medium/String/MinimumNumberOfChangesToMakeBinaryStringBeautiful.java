...
/**
 * <a class="no-underline hover:text-blue-s dark:hover:text-dark-blue-s truncate cursor-text whitespace-normal hover:!text-[inherit]" href="/problems/minimum-number-of-changes-to-make-binary-string-beautiful/">2914. Minimum Number of Changes to Make Binary String Beautiful</a>
 * 
 * <div><div class="elfjS" data-track-load="description_content"><p>You are given a <strong>0-indexed</strong> binary string <code>s</code> having an even length.</p>
 * 
...
public class MinimumNumberOfChangesToMakeBinaryStringBeautiful {
    public static void main(String[] args) {
        ...
        for (String s : tests) {
            System.out.println(new MinimumNumberOfChangesToMakeBinaryStringBeautiful_Solution().minChanges(s));
        }
    }
}

class MinimumNumberOfChangesToMakeBinaryStringBeautiful_Solution {
    public int minChanges(String s) {
        int len = s.length(),
            min_step = 0;
        for (int i = 0; i < len; i+= 2) {
            char pre = s.charAt(i);
            if (pre != s.charAt(i+1)) { min_step++; }
        }   
